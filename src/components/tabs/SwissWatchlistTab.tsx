...
const VERDICT_COLORS: Record<string, string> = {
  ...
  "reasonably priced": "bg-warning/15 text-warning border-warning/30",
...
export default function SwissWatchlistTab({ data, loading }: Props) {
  ...
  if (!data || data.rows.length === 0) {
    return (
      <Card className="flex h-64 items-center justify-center border-border bg-card">
        <p className="text-muted-foreground">No "Swiss Watchlist" sheet found</p>
      </Card>
    );
  }
    ...
    <div className="animate-fade-in space-y-6">
      ...
      <Card className="overflow-x-auto border-border bg-card p-4">
        <div className="mb-3 flex items-center justify-between">
          <h3 className="font-display text-sm font-semibold uppercase tracking-wider text-muted-foreground">Swiss Watchlist — Researched Securities</h3>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b-2 border-border">
              ...
              {sectorIdx !== -1 && <SortableHeader label="Sector" colIdx={sectorIdx} sort={sort} onToggle={toggleSort} className="text-left" />}
            ...
            {sortedRows.map((row, i) => {
              ...
              return (
                <tr key={i} className="border-b border-border/30 transition-colors hover:bg-secondary/30">
                  ...
                  {sectorIdx !== -1 && <td className="px-3 py-3 text-muted-foreground">{row[sectorIdx]}</td>}
